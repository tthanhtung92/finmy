...
- **Chạy seeder lúc startup**: cách sạch là một `IHostedService` đăng ký trong `AddInfrastructure`; host tự gọi `StartAsync` trước khi mở cổng nhận request. `RoleManager`/`UserManager` là **scoped** nên hosted service (singleton) không inject thẳng được — inject `IServiceScopeFactory`, `CreateScope()` rồi resolve trong scope. Cách thay thế đơn giản hơn: mở scope tay trong `Program.cs` (`app.Services.CreateScope()`) trước `app.Run()`. Nguồn: [Dependency injection: resolving scoped services at startup](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/dependency-injection?view=aspnetcore-10.0#scope-validation).

## 5.4. Các bước làm

1. **Seeder (Infrastructure):** viết seeder ở `Infrastructure/Persistence/` (vd static class `IdentitySeeder`) với mỗi role trong danh sách (`Admin`, `User`), `RoleExistsAsync` false thì `CreateAsync(new ApplicationRole { Name = ... })`. Tùy chọn: tạo một user Admin mặc định rồi `AddToRoleAsync(admin, "Admin")` để có sẵn tài khoản Admin demo. Hai điểm phải nhớ: **check `IdentityResult.Succeeded`** sau mỗi `CreateAsync`/`AddToRoleAsync` (không thì fail im lặng — password rớt policy vẫn trôi), và **credential admin đọc từ config** (User Secrets/env), không hardcode password vào source. Gợi ý gom `IsSeedAdmin`/`AdminUserName`/`AdminEmail`/`AdminPassword` vào một Options class bind qua `Configure<T>` hoặc `GetSection(...).Get<T>()`.
2. **Gọi seeder ở host:** cách khuyến nghị là `IHostedService` (đăng ký `AddHostedService<...>` trong `AddInfrastructure`) — `StartAsync` mở scope qua `IServiceScopeFactory`, resolve `RoleManager`/`UserManager`, await seeder. `Program.cs` không phải đụng gì, seeding đóng gói trong module Identity. Cách thay thế: gọi tay trong `Program.cs` giữa `Build()` và `Run()`.
