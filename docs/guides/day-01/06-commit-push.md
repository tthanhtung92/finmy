...
## 6.1. Build & chạy lần cuối (cổng chất lượng)
...
## 6.2. Xem lại những gì sắp commit

```bash
git status
```

Đọc danh sách. Bạn nên thấy: `Directory.Packages.props`, `Directory.Build.props` (mới), các `.csproj` đã sửa, `Program.cs` đã sửa, 2 project Shared mới, README sửa, và các `Class1.cs` bị xóa.
...
## 6.3. Tách thành vài commit nhỏ (kể chuyện rõ hơn)
...
1. Nền tảng build: `Directory.Packages.props` + `Directory.Build.props` + dọn `.csproj`.
2. Project Shared: thêm `SharedKernel` + `Contracts`.
3. Dọn template + sửa README.

Với mỗi nhóm: chọn file (`git add <đường-dẫn>`) rồi commit. Ví dụ luồng cho nhóm 1:

```bash
git add Directory.Packages.props Directory.Build.props src/**/*.csproj
git commit -m "build: thêm Central Package Management và build settings dùng chung"
```

Làm tương tự cho nhóm 2 và 3 với message phù hợp.
...
## 6.4. Quy ước commit (Conventional Commits, tiếng Việt)
...
- `docs:` — thay đổi tài liệu (sửa README).
- `feat(...)`/`fix(...)` — để dành cho khi viết tính năng/sửa lỗi thật ở các ngày sau.

Ví dụ tốt: `chore: dọn code template và Class1.cs`, `docs: cập nhật badge và link clone trong README`.
...
## 6.7. Hoàn thành Day 1 🎉

Đối chiếu lại [checklist Definition of Done ở trang chỉ mục Day 1](README.md). Nếu đủ hết, nhắn mentor **"review Day 1"** — mình sẽ rà `Directory.Packages.props`, `Directory.Build.props`, các `.csproj`, `Program.cs` và `.slnx` của bạn, rồi mới mở [Day 2](../README.md).

> **Tự vấn nhanh (quan trọng hơn code):** bạn có giải thích trôi chảy cho người khác nghe được không — CPM giải quyết vấn đề gì, Directory.Build.props nạp lúc nào, và vì sao module không được tham chiếu trực tiếp Domain của module khác? Nếu chưa, đọc lại [Bước A](00-hieu-scaffold.md). Project này ăn điểm ở chỗ *giải thích được*, không chỉ *làm được*.
