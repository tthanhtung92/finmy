# Bước 2 — Build settings dùng chung (`Directory.Build.props`)
...
## 2.1. Vấn đề (vì sao làm bước này)

Mở [EventHub.Identity.Domain.csproj](../../../src/Modules/Identity/EventHub.Identity.Domain/EventHub.Identity.Domain.csproj) — bạn thấy nó tự khai `TargetFramework` là `net10.0`, `Nullable` là `enable`, `ImplicitUsings` là `enable`. Bây giờ mở các `.csproj` khác: y hệt, lặp đi lặp lại. Khi có chục project, đổi một thiết lập (vd bật "coi warning là lỗi") phải sửa chục file — dễ sót.
...
Tạo file tên `Directory.Build.props` ở **gốc repo** (ngang `EventHub.slnx` và `Directory.Packages.props`). Không có `dotnet new` riêng cho nó — tạo file trống rồi điền theo mục 2.3.

## 2.3. Nội dung nên đưa vào (tự gõ XML)
...
## 2.4. Dọn trùng lặp trong các `.csproj` (đừng quên bước này)

Sau khi đưa các thuộc tính trên vào `Directory.Build.props`, hãy **mở từng `.csproj`** (host, 4 project Identity) và **xóa các thuộc tính đã trùng** (`TargetFramework`, `Nullable`, `ImplicitUsings`). Vì chúng đã nằm ở file gốc, để lại là khai báo hai nơi.
...
- Phải `Build succeeded`. Nếu bạn bật `TreatWarningsAsErrors` và build đỏ vì vài warning của code template còn sót — tốt, [Bước 4](04-don-template.md) sẽ dọn chúng; tạm thời cứ ghi nhận.
...
- [ ] Các `.csproj` đã bỏ thuộc tính trùng, gọn lại.
- [ ] `dotnet build EventHub.slnx` xanh (hoặc chỉ đỏ vì code template — sẽ dọn ở Bước 4).

→ Sang [Bước 3 — Tạo project Shared](03-shared-projects.md).
