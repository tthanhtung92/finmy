# ADR-0006: Chuyển domain sang ngân sách chia sẻ (shared personal finance) theo envelope budgeting
...
Finmy là project học tập, một người làm, mục tiêu là mỗi khái niệm backend cốt lõi (auth, caching, CDN, realtime, messaging, concurrency) có một vertical slice mỏng nhưng chạy thật và *giải thích được*. Bài toán "đinh" xuyên suốt là chống tranh chấp khi nhiều người cùng ghi lên một tài nguyên khan hiếm — trong domain gốc là chống overselling vé.
...
1. Domain phải là thứ tác giả **thật sự vận hành** (self-host, tự dùng cho một nhóm chia sẻ thật), để có động lực giữ nó đúng và hoàn thiện dần.
...
- **Ngân sách chia sẻ (shared personal finance) theo envelope budgeting**: một nhóm cùng tiêu chung, chia tiền vào các "phong bì" ngân sách. Có nhu cầu dùng thật hằng ngày; thoát ràng buộc 3 nhờ ba lý do: dữ liệu tiền bạc nên tự host vì privacy, nhu cầu nội địa (VND, ngân hàng và e-wallet VN) không được YNAB/Actual phục vụ tốt, và bản tự viết may đo được cho đúng nhóm mình. Firefly III / Actual / Money Lover tồn tại nhưng lực kéo "thôi dùng luôn cái có sẵn" ở đây yếu hẳn.

Trong nhánh finance còn hai lựa chọn con:

- **Solo tracking vs nhóm chia sẻ.** Solo tracking về bản chất là CRUD thuần, bài toán concurrency phải gắn vào một cách gượng ép. Nhóm chia sẻ với ngân sách chung thì concurrency và realtime có ý nghĩa thật (nhiều thành viên cùng chi một phong bì).
...
Chúng tôi chuyển domain của Finmy sang **ngân sách chia sẻ (shared personal finance) theo mô hình envelope budgeting**, với **ledger single-entry**, giữ trọn 8 khái niệm và phạm vi MVP full 8-concept.

- Aggregate trung tâm là **Space** (root cho việc chia sẻ), sở hữu Account, Category, Envelope và Transaction. **Member** gắn một user vào Space kèm role (Owner / Member / Viewer); ranh giới authorization là user chỉ chạm dữ liệu của Space mình. Space là một nhóm chia sẻ phẳng, không gắn với một loại nhóm cụ thể: một nhóm bạn ở ghép, một quỹ chuyến đi chung, hay một đội nhóm nhỏ đều là một Space.
...
- Ánh xạ các slice khái niệm: caching là cache report/aggregate kèm invalidation; CDN / object-storage là MinIO lưu ảnh hóa đơn; realtime là SignalR đẩy số dư mới cho các thành viên trong cùng một Space; messaging là post transaction async cộng recurring transaction qua Wolverine scheduled cộng transactional outbox; concurrency là rowversion trên Envelope; idempotency là dedup khi import CSV sao kê.
- Phạm vi MVP: **full 8-concept**, bao gồm cả MinIO và recurring. Ledger **single-entry**.

## Hệ quả

- Domain nay có driver "tác giả vận hành thật" (tự host ngân sách chia sẻ của mình), khác hẳn domain dùng một lần: áp lực giữ nó đúng đến từ chính việc mình phải xài.
